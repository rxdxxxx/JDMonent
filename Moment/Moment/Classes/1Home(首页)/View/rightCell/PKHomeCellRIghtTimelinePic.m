...
@interface PKHomeCellRIghtTimelinePic()
...
@implementation PKHomeCellRIghtTimelinePic

+ (instancetype)cellWithTableView:(UITableView *)tableView
{
    static NSString *ID = @"PKHomeCellRIghtTimelinePic";
    PKHomeCellRIghtTimelinePic *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (cell == nil) {
        
        cell = [[[NSBundle mainBundle]loadNibNamed:@"PKHomeCellRIghtTimelinePic" owner:nil options:nil] lastObject];
...
-(void)setRightModel:(PKHomeModelFeedRoot *)rightModel
{
    ...
    // 6,内容
    _contentLabel.text = rightModel.content;
